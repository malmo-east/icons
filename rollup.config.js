...
const currentComponentName = pkg.name.replace('@friday-team/icons-', '');
...
const copyPlugin = dest =>
    copy({
        flatten: false,
        targets: [{ src: ['src/**/*.{png,svg,jpg,jpeg}'], dest }],
    });
    ...
    output: [
        {
            dir: 'dist/modern',
            format: 'esm',
            plugins: [coreComponentsResolver({ importFrom: 'dist/modern' })],
        },
    ],
    plugins: [
        multiInputPlugin,
    ...
    output: [
        {
            dir: 'dist/esm',
            format: 'esm',
            plugins: [coreComponentsResolver({ importFrom: 'dist/esm' })],
        },
    ],
    plugins: [
        multiInputPlugin,
...
const configs = [es5, modern, esm, cssm, root].filter(Boolean);
