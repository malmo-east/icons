import * as React from "react";
import { SVGProps } from "react";

const SvgFaceLaughSquint = (props: SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" {...props}>
    <path
      className="face-laugh-squint_svg__fa-primary"
      d="M116 125.1c0-8.9 9.6-14.7 17.5-10.4l89.9 47.9c10.7 5.7 10.7 21.1 0 26.8l-89.9 47.9c-7.9 4.3-17.5-1.5-17.5-10.4 0-2.8.1-5.5 2.8-7.7l36-43.2-36-43.2c-2.7-2.2-2.8-4.9-2.8-7.7zm277.2 7.7-36 43.2 36 43.2c1.8 2.2 2.8 4.9 2.8 7.7 0 8.9-9.6 14.7-17.5 10.4l-89.9-47.9c-10.7-5.7-10.7-21.1 0-26.8l89.9-47.9c7.9-4.3 17.5 1.5 17.5 10.4 0 2.8-1 5.5-2.8 7.7z"
    />
    <path
      d="M0 256C0 114.6 114.6 0 256 0s256 114.6 256 256-114.6 256-256 256S0 397.4 0 256zm256 176c76.1 0 140.2-50 159.2-117.9 3.9-13.7-7.4-26.1-21.6-26.1H118.4c-14.2 0-25.48 12.4-21.64 26.1C115.8 382 179.9 432 256 432zM133.5 114.7c-7.9-4.3-17.5 1.5-17.5 10.4 0 2.8.1 5.5 2.8 7.7l36 43.2-36 43.2c-2.7 2.2-2.8 4.9-2.8 7.7 0 8.9 9.6 14.7 17.5 10.4l89.9-47.9c10.7-5.7 10.7-21.1 0-26.8l-89.9-47.9zM396 125.1c0-8.9-9.6-14.7-17.5-10.4l-89.9 47.9c-10.7 5.7-10.7 21.1 0 26.8l89.9 47.9c7.9 4.3 17.5-1.5 17.5-10.4 0-2.8-1-5.5-2.8-7.7l-36-43.2 36-43.2c1.8-2.2 2.8-4.9 2.8-7.7z"
      style={{
        opacity: 0.4,
      }}
    />
  </svg>
);

export default SvgFaceLaughSquint;
